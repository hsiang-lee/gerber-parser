...
ParameterParser::ParameterParser(GerberParser& gerber) :gerber_parser_(gerber) {
...
bool ParameterParser::Run() {
	auto delimiter = gerber_parser_.gerber_file_->buffer_[gerber_parser_.gerber_file_->pointer_ - 1];
	...
	while (gerber_parser_.gerber_file_->MoreThanLastOne()) {
		...
		if (parsers_.find(code) != parsers_.end()) {
			...
			LOG(ERROR) << "Line " << gerber_parser_.gerber_file_->line_number_ << " - Error: Unknown parameter: " << code;
			return false;
		}
	...
	LOG(ERROR) << "Line " << gerber_parser_.gerber_file_->line_number_ << " - Error: Parameter Block without end-delimiter";
