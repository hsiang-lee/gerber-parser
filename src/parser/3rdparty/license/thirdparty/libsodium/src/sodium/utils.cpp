...
void _sodium_dummy_symbol_to_prevent_memzero_lto(void *const pnt, const size_t len);
void _sodium_dummy_symbol_to_prevent_memzero_lto(void *const pnt, const size_t len) {
...
#ifdef HAVE_WEAK_SYMBOLS
void _sodium_dummy_symbol_to_prevent_memcmp_lto(const unsigned char *b1, const unsigned char *b2, const size_t len);
void _sodium_dummy_symbol_to_prevent_memcmp_lto(const unsigned char *b1, const unsigned char *b2, const size_t len) {
  (void)b1;
  (void)b2;
  (void)len;
...
#ifdef HAVE_WEAK_SYMBOLS
void _sodium_dummy_symbol_to_prevent_compare_lto(const unsigned char *b1, const unsigned char *b2, const size_t len);
void _sodium_dummy_symbol_to_prevent_compare_lto(const unsigned char *b1, const unsigned char *b2, const size_t len) {
  (void)b1;
  (void)b2;
  (void)len;
...
void sodium_sub(unsigned char *a, const unsigned char *b, const size_t len) {
  ...
  if (len == 64U) {
    __asm__ __volatile__(
        ...
        "sbbq %[t64_7], 48(%[out]) \n"
        "sbbq %[t64_8], 56(%[out]) \n"
        : [t64_1] "=&r"(t64_1), [t64_2] "=&r"(t64_2), [t64_3] "=&r"(t64_3), [t64_4] "=&r"(t64_4), [t64_5] "=&r"(t64_5), [t64_6] "=&r"(t64_6),
          [t64_7] "=&r"(t64_7), [t64_8] "=&r"(t64_8)
        : [in] "S"(b), [out] "D"(a)
        : "memory", "flags", "cc");
    return;
  }
...
static void *_sodium_malloc(const size_t size) { return malloc(size > (size_t)0U ? size : (size_t)1U); }
#else
static void *_sodium_malloc(const size_t size) {
  void *user_ptr;
  ...
  if ((base_ptr = _alloc_aligned(total_size)) == nullptr) {
...
void *sodium_malloc(const size_t size) {
  ...
  if ((ptr = _sodium_malloc(size)) == nullptr) {
    return nullptr;
  }
...
#ifndef HAVE_PAGE_PROTECTION
static int _sodium_mprotect(void *ptr, int (*cb)(void *ptr, size_t size)) {
  (void)ptr;
  (void)cb;
  errno = ENOSYS;
  return -1;
...
int sodium_mprotect_noaccess(void *ptr) { return _sodium_mprotect(ptr, _mprotect_noaccess); }

int sodium_mprotect_readonly(void *ptr) { return _sodium_mprotect(ptr, _mprotect_readonly); }
...
int sodium_pad(size_t *padded_buflen_p, unsigned char *buf, size_t unpadded_buflen, size_t blocksize, size_t max_buflen) {
  ...
  if (padded_buflen_p != nullptr) {
    *padded_buflen_p = xpadded_len + 1U;
  }
  mask = 0U;
  for (i = 0; i < blocksize; i++) {
    barrier_mask = (unsigned char)(((i ^ xpadlen) - 1U) >> ((sizeof(size_t) - 1) * CHAR_BIT));
