...
allocate_memory(block_region **region, uint32_t m_cost)
{
    ...
    *region = (block_region *) malloc(sizeof(block_region));
    ...
    (*region)->base = (*region)->memory = nullptr;

#if defined(MAP_ANON) && defined(HAVE_MMAP)
    if ((base = mmap(nullptr, memory_size, PROT_READ | PROT_WRITE,
                     MAP_ANON | MAP_PRIVATE | MAP_NOCORE | MAP_POPULATE,
                     -1, 0)) == MAP_FAILED) {
        base = nullptr; /* LCOV_EXCL_LINE */
    }                /* LCOV_EXCL_LINE */
    memory = (block *) base;
    ...
    if (memory_size + 63 < memory_size) {
        base  = nullptr;
        errno = ENOMEM;
    } else if ((base = malloc(memory_size + 63)) != nullptr) {
                    ...
                    argon2_type type)
{
    ...
    crypto_generichash_blake2b_init(&BlakeHash, nullptr, 0U,
...
argon2_initialize(argon2_instance_t *instance, argon2_context *context)
{
    ...
    if ((instance->pseudo_rands = (uint64_t *)
         malloc(sizeof(uint64_t) * instance->segment_length)) == nullptr) {
        return ARGON2_MEMORY_ALLOCATION_ERROR;
    }
