...
{
    ctx->input[12] = counter == nullptr ? 0 : LOAD32_LE(counter + 0);
    ctx->input[13] = counter == nullptr ? 0 : LOAD32_LE(counter + 4);
...
{
    ctx->input[12] = counter == nullptr ? 0 : LOAD32_LE(counter);
        ...
        if (bytes <= 64) {
            if (bytes < 64) {
                for (i = 0; i < (unsigned int) bytes; ++i) {
                    ctarget[i] = c[i]; /* ctarget cannot be nullptr */
                }
            }
            ctx->input[12] = j12;
            ctx->input[13] = j13;
                    ...
                    const unsigned char *n, const unsigned char *k)
{
    ...
    COMPILER_ASSERT(crypto_stream_chacha20_KEYBYTES == 256 / 8);
    chacha_keysetup(&ctx, k);
    chacha_ietf_ivsetup(&ctx, n, nullptr);
    memset(c, 0, clen);
    chacha20_encrypt_bytes(&ctx, c, c, clen);
    sodium_memzero(&ctx, sizeof ctx);
