...
template <typename String, unsigned int LineLength>
static std::string EncodeWithLineBreaks(String s) {
    return InsertLineBreaks(Base64Encode(s, false), LineLength);
}

template <typename String>
static std::string EncodePem(String s) {
    return EncodeWithLineBreaks<String, 64>(s);
}

template <typename String>
static std::string EncodeMime(String s) {
...
std::string Base64Encode(unsigned char const* bytes_to_encode, size_t in_len,
    ...
    //
    const char* base64_chars = g_base64_chars[url];
...
static std::string Decode(String encoded_string, bool remove_linebreaks) {
    ...
    while (pos < in_len) {
        unsigned int pos_of_char_1 = PosOfChar(encoded_string[pos + 1]);

        ret.push_back(static_cast<std::string::value_type>(
            ((PosOfChar(encoded_string[pos + 0])) << 2) +
        ...
        if (encoded_string[pos + 2] != '=' &&
            ...
            unsigned int pos_of_char_2 = PosOfChar(encoded_string[pos + 2]);
            ret.push_back(static_cast<std::string::value_type>(
            ...
            if (encoded_string[pos + 3] != '=' &&
                ...
                ret.push_back(static_cast<std::string::value_type>(
                    ...
                    PosOfChar(encoded_string[pos + 3])));
            }
        }
...
std::string Base64Encode(std::string const& s, bool url) {
    return Encode(s, url);
}

std::string Base64EncodePem(std::string const& s) { return EncodePem(s); }

std::string Base64EncodeMime(std::string const& s) { return EncodeMime(s); }
...
std::string Base64Encode(std::string_view s, bool url) {
    return Encode(s, url);
}

std::string Base64EncodePem(std::string_view s) { return EncodePem(s); }

std::string Base64EncodeMime(std::string_view s) { return EncodeMime(s); }

std::string Base64Decode(std::string_view s, bool remove_linebreaks) {
    return Decode(s, remove_linebreaks);
}
