...
void MD5Update(MD5Ctx *context, unsigned char *input, unsigned int input_len) {
    ...
    if ((context->count[0] += (static_cast<UINT4>(input_len) << 3)) < (static_cast<UINT4>(input_len) << 3)) {
        context->count[1]++;
    }
    context->count[1] += (static_cast<UINT4>(input_len) >> 29);

    part_len = 64 - index;

    // Transform as many times as possible.
    if (input_len >= part_len) {
        MD5Memcpy(&context->buffer[index], input, part_len);
    ...
    MD5Memcpy(&context->buffer[index], &input[i], input_len - i);
}
...
void MD5Final(unsigned char digest[16], MD5Ctx *context) {
    ...
    MD5Memset(reinterpret_cast<POINTER>(context), 0, sizeof(*context));
}
...
static void MD5Transform(UINT4 state[4], unsigned char block[64]) {
    ...
    // Zeroize sensitive information.
    MD5Memset(reinterpret_cast<POINTER>(x), 0, sizeof(x));
}
